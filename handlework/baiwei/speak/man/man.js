...
Page({
  ...
  tomanage() {
    wx.navigateTo({
      url: '/speak/managenormalusesentence/managenormalusesentence',
    })
  },
  toaddnormal() {
    wx.navigateTo({
  ...
  getList() {
    wx.request({
      url: 'https://51favlife.com/index.php?s=/server/index/chatlog',
      data: {
        from: this.data.fromId,
        to: this.data.toId
      },
      success: (res) => {
        ...
        if (res.data.code == 1) {
          for (let i in res.data.data) {
            if (res.data.data[i].type == 'photo') {
              res.data.data[i].message = '<img src="' + res.data.data[i].message + '" style="max-width:100px;max-height:100px;"/>'
            }
          }
          this.setData({
  ...
  forws(){
    app.wsobj = wx.connectSocket({
      url: 'wss://51favlife.com/wss?user_id=' + wx.getStorageSync('user_id'),
      success: (res) => {
  ...
  sendmessage(data, success = null) {
    ...
    wx.request({
      url: 'https://51favlife.com/index.php?s=/server/index/send',
      method: 'POST',
      header: {
