<template>
  <div class="checkCenter contain">
    <div v-if="this.$route.path == '/inCheckCenterCheck'">
      <div class="top topBtns">
        <div class="leftBtns" v-if="newshenpi">
      ...
      <div style="height: 20px;text-align:right;">
        <el-link :underline='fabecomedetaillse' v-if="tabbar == 1" @click="delAll"><i class='el-icon-delete'>批量删除</i></el-link>
      </div>
          ...
          <template slot-scope="scope">
            <div class="action">
              <menRadio :excuters="excuters" @theRes="execuRes">
                ...
                <template slot="main">
                  <el-link :underline='false' type='primary' style="margin-right:20px;" @click="getRowId(scope.row.id)">
                    分享 <i class='el-icon-top-right'></i></el-link>
                </template>
              </menRadio>
  ...
  export default {
    ...
    methods: {
      ...
      execuRes(res) {
        ...
        this.$http.post('approval/sharewilldo', {
          approval_id: res.id,
    ...
    created() {
      this.newshenpi = this.$http.allow('新建审批')
      // 审批类型
