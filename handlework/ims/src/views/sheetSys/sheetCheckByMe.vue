<template>
  <div class="sheetSys contain">
    <div v-if="this.$route.path == '/sheetCheckByMe'">
      <div class="topBtns">
      ...
      :header-cell-style="$store.state.headStyle">
        ...
        <el-table-column label="优先级" prop="priority" width="70"></el-table-column>
        <el-table-column label="执行部门" prop="structure"></el-table-column>
        <el-table-column label="执行人" prop="executor" width="70"></el-table-column>
        ...
        <el-table-column label="负责人" prop="charge" width="70"></el-table-column>
        <el-table-column label="审批状态" prop="order_status"></el-table-column>
...
<script>
  export default {
    ...
    methods: {
      ...
      search(value) {
        this.$http.post('workorder/searchmyexecutor', { token: localStorage.getItem('token'), keyword: value }).then(res => {
      ...
      getList(page) {
        this.$http.get('workorder/approval', { token: localStorage.getItem('token'), page: page || 1 }).then(res => {
          // this.tableData = res.msg
          // this.tempData = res.msg.slice(0,10)
          this.res = res
