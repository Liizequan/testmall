<template>
  <div class="sheetSys contain">
    <div v-if="this.$route.path == '/mySheet'">
      <div class="topBtns">
...
<script>
  export default {
    ...
    methods: {
      ...
      toDetail(id) {
        this.$router.push({ path: "/sheetSys/sheetDetl", query: { id: id ,iamthereceiver:true} });
      },
      getList(page) {
        this.$http.get('workorder/myexecutor', { token: localStorage.getItem('token'), page: page || 1 }).then(res => {
          // this.tableData = res.msg
          // this.tempData = res.msg.slice(0,10)
          this.res = res
