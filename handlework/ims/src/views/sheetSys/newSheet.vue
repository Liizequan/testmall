<template>
  <div class="newSheet contain">
    <div class="topTitle">
      <h4>{{who=='edit'?'编辑':'新建'}}工作单</h4>
    </div>
    <div class="line_title">基础信息</div>
    <el-form label-width="120px">
      <div>
        <el-form-item label="项目名称:" class="itemWidth redStar">
          <el-input placeholder="请填项目名称（15字之内）" v-model="name" maxlength="15"></el-input>
        </el-form-item>
      </div>
      <div>
        <el-form-item label="工单类型:" class="itemWidth redStar">
      ...
      <div>
        <el-form-item label="工作内容:" class="itemWidth redStar">
          <el-input type="textarea" :rows="8" v-model="content" maxlength="255" placeholder='请填工作内容（255字之内）'></el-input>
        </el-form-item>
      </div>
      <div>
        <el-form-item>
          <el-popover placement="bottom-start" trigger="click" v-model="popoVisi" @hide="confirm">
            ...
            <div slot="reference" style="width:100px;">
              <el-link :underline="false" type="primary "><i class="el-icon-link"></i>关联客户</el-link>
            </div>
          </el-popover>
      ...
      <div style="color:#409EFF;">
        <el-form-item class="itemWidth" v-for="(item,index) in selected" :key="index" v-if="item.group">
          <i class="el-icon-link leftIcon"></i>
          <span class="category">{{item.group}}</span><span>-</span>
          <span>{{item.name}}</span><i class="el-icon-delete rightIcon" @click="delSel(index)"></i>
        </el-form-item>
      ...
      <div>
        <el-form-item label="执行人:" class="itemWidth redStar">
          <el-popover placement="bottom-start" trigger="click" v-model="excuPopo" @hide="isexcuConf">
            <div>
              <h3>执行人</h3>
                ...
                <el-collapse-item>
                  ...
                  <div>
                    <el-checkbox-group v-model="searchSeled">
                      <ul style="display: flex;flex-direction:column;">
                        <li style="height:30px;line-height: 30px;" v-for="(item,index) in memberRes" :key="index">
      ...
      <div>
        <el-form-item label="执行部门:" class="itemWidth redStar">
          <el-popover placement="bottom-start" trigger="click" v-model="departPopo" @hide="departConf">
            <h3>执行部门</h3>
            <!-- <el-checkbox-group v-model="departIdsArr" class="checkGroup">
              <el-checkbox v-for="item in departs" :key="item.id" :label="item.id" v-if="item.id">{{item.label}}
              </el-checkbox>
            </el-checkbox-group> -->
            <el-checkbox-group v-model="departIdsArr" class="checkGroup xyk-scroll" style="height:250px;overflow:auto;">
              <el-checkbox v-for="item in departs" :key="item.id" :label="item.id" v-if="item.id"
                style="padding:4px 0;">{{item.label}}
              </el-checkbox>
            </el-checkbox-group>
            <div class="popoBtns">
              <el-link :underline="false"><span class="popoCon" @click="departConf">确定</span></el-link>
            ...
            <div slot="reference" class="Excu">
              <el-input placeholder="请选择执行部门" v-model="departSelNameStr"></el-input>
            </div>
          </el-popover>
        </el-form-item>
      </div>
      <div>
        <el-form-item label="优先级:" class="itemWidth redStar">
          <!-- <el-button round plain class="danger" :class="prioIndex==0?'prioBg':''" @click="selePrio(0)">紧急</el-button>
          <el-button round plain class="orange" :class="prioIndex==1?'prioBg':''" @click="selePrio(1)">正常</el-button>
          <el-button round plain class="blue" :class="prioIndex==2?'prioBg':''" @click="selePrio(2)">缓慢</el-button> -->
          <div class="danger" :class="prioIndex==0?'oneSel':''" @click="selePrio(0)">紧急</div>
          <div class="orange" :class="prioIndex==1?'twoSel':''" @click="selePrio(1)">正常</div>
          <div class="blue" :class="prioIndex==2?'threeSel':''" @click="selePrio(2)">缓慢</div>
        </el-form-item>
      </div>
      <div>
        <el-form-item label="开始时间:" class="itemWidth redStar">
          <el-date-picker v-model="start_time" class="el_date" placeholder='请选择'></el-date-picker>
        </el-form-item>
      </div>
      <div>
        <el-form-item label="预计结束时间:" class="itemWidth redStar">
          <el-date-picker v-model="expect_time" class="el_date" placeholder='请选择'></el-date-picker>
        </el-form-item>
      </div>
      <div>
        <el-form-item label="实际结束时间:" class="itemWidth">
          <el-date-picker v-model="real_time" class="el_date" placeholder='请选择'></el-date-picker>
        </el-form-item>
      </div>
      <div>
        <el-form-item label="任务状态:" class="itemWidth redStar">
          <el-select placeholder="请选择" class="select" v-model="status" @change="hahaha">
            <el-option v-for="(item,index) in statuses" :key="index" :label="item.label" :value="item.id"></el-option>
          </el-select>
        </el-form-item>
      </div>
      <div>
        <el-form-item label="负责人:" class="itemWidth redStar">
          <el-popover placement="bottom-start" trigger="click" v-model="chargePopo">
            <div>
              <h3>负责人</h3>
              <el-input placeholder="搜索成员" @change="searchMenber" v-model="memberKey"></el-input>
              <el-collapse accordion style="height:300px;border-bottom:0;overflow:auto;" class="xyk-scroll">
                <el-collapse-item>
                  <template slot="title">
                    <div>
                      搜索结果
                  ...
                  <div>
                    ...
                    <!-- <el-checkbox-group v-model="searchSeled">
                      <el-checkbox  class="flex-items-center" v-for="(item,index) in memberRes" :key="index" :label="item.name">
            ...
            <div slot="reference" style="width:100px;" class="Excu">
              <el-input placeholder="请选择负责人" v-model="response"></el-input>
            </div>
          </el-popover>
        </el-form-item>
      </div>
      <div>
        <el-form-item label="审批人:" class="itemWidth redStar">
          <el-popover placement="bottom-start" trigger="click" v-model="dispaPopo">
            <div>
              ...
              <el-radio-group v-model="theDispRadio" style="flex-direction: column;">
                <el-collapse accordion style="height:300px;border-bottom:0;overflow:auto;" class="xyk-scroll">
                  <el-collapse-item>
                    <template slot="title">
                      <div>
                  ...
                  <el-collapse-item v-for="(item,index) in excuters" :key="index">
                    ...
                    <div>
                      ...
                      <ul style="display: flex;flex-direction:column;">
                        <li style="height:30px;line-height: 30px;" v-for="it in item.user" :key="it.user_id">
                          <el-radio :label="it.user_id" style="display:block">
                            {{it.name}}
                          </el-radio>
                        </li>
                      </ul>
            ...
            <div slot="reference">
              <el-input placeholder="请选择审批人" v-model="distru"></el-input>
            </div>
          </el-popover>
        </el-form-item>
      </div>
      <div>
        <el-form-item class="itemWidth">
          <i class="el-icon-plus" style="color:#44bcf0;">上传附件</i><span
            style="color:#ccc;">（只能上传一份,格式为rar/zip/doc/docx/pdf）</span>
          <input type="file" name="file" @change="fileSel" ref="file" id="file">
        </el-form-item>
      </div>
      <div>
        <el-form-item class="itemWidth">
          <!-- <i class='el-icon-folder' v-if="who=='edit'&&files.length>0">{{editFile?editFile.name:''}}</i> -->
          <!-- <i class='el-icon-folder' v-else v-for="(item,index) in files" :key="index">{{item.name}}</i> -->
          <div v-for="(item,index) in files" :key="index" v-if="files&&files.length>0"
            ...
            <div @click="deleteFile(index)">删除<i class='el-icon-delete'></i></div @click="deleteFile(index)">
          </div>
        </el-form-item>
      </div>
    </el-form>
  ...
  export default {
    data() {
      return {
        ...
        editId: '',
        who: '',
        receiverEditingStatus: false
      };
    },
    methods: {
      hahaha() {
        console.log('接收人员在修改任务状态咯')
        this.receiverEditingStatus = true
      },
      deleteFile(index) {
      ...
      excuConf() {
        this.excuSelIdArr = []
        this.excuSelIdArr = [...this.excuSelIdArr, ...this.searchSeled]
        this.excuter = ''
        for (let i in this.excuters) {
      ...
      chargeConf() {
        this.chargeSelIdArr = []
        this.chargeSelIdArr = [...this.chargeSelIdArr, ...this.searchSeled]
        this.response = ''
        for (let i in this.excuters) {
      ...
      receiverEdit() {
        let status
        if (this.receiverEditingStatus) {
          status = this.status
        } else {
          status = this.getStatusIndex()
        }
        this.$http.post('workorder/getSheetManEdit', {
          id: this.editId,
      ...
      save() {
        ...
        let reqBody = {
          ...
          // dispatch: this.theDispRadio,//派单人
          dispatch: JSON.parse(localStorage.getItem('login_status')).info.user_id,
          // file: this.who == 'edit' ? (this.editFile?this.editFile.id:'') : this.$http.getIdsStr(this.fileIds),
          file: this.$http.getIdsStr(this.fileIds),
          approval: this.theDispRadio
          // status:this.status
        }
        if (this.who == 'edit') {
          api = 'workorder/edit'
      ...
      getStatusIndex() {
        console.log(this.status, '111111111111')
      ...
      editInit() {
        this.$http.get('workorder/look', { token: localStorage.getItem('token'), id: this.editId }).then(res => {
          ...
          this.dispConfirm()
          this.editFile = res.file ? res.file : []
          this.fileIds = []
          if (res.file) {
    ...
    created() {
      ...
      if (this.iamthereceiver) {
        this.$message('保存后只会对实际结束时间和任务状态进行修改')
      }
      this.getDepart()
