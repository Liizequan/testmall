        ...
        <div class="form">
            <el-form :label-position="labelPosition" label-width="170px" :model="formLabelAlign">
                ...
                <el-form-item label="工号 :">
                    <el-input v-model="formLabelAlign.region" maxlength="10"></el-input>
                </el-form-item>
                ...
                <Approval :leftText="leftText" :approvalId = 'approvalId'  @approData="getApprovalData" />
            </el-form>
        </div>
        <div>
    ...
    export default {
        ...
        methods: {
            ...
            saveData(){
                ...
                var add_relation = Object.assign({textarea:this.textarea,summary:this.summary,suggestions:this.suggestions,approvalId:this.approvalId},formLabelAlign)
                console.log(add_relation)
                localStorage.setItem("add_relation", JSON.stringify(add_relation) );
                this.back();
                this.$message('保存成功')
            },
            // 不暂存清除数据
        ...
        created() {
            ...
            console.log(this.$route.query.id)
            var add_relation = localStorage.getItem('add_relation')
            console.log(add_relation)
            if(add_relation){
                this.addsave = true,
                add_relation = JSON.parse(add_relation)
                console.log(add_relation)
                // this.formLabelAlign = add_relation;
                // this.textarea = add_relation.textarea 
                // this.summary = add_relation.summary
                // this.suggestions = add_relation.suggestions
                // this.approvalId = add_relation.approvalId
                this.formLabelAlign.name = add_relation.name//申请人
                this.formLabelAlign.region = add_relation.region//工号
                this.formLabelAlign.positionId = parseInt(add_relation.positionId)//（职位id）
                this.position = parseInt(add_relation.positionId)
                this.formLabelAlign.structureId = parseInt(add_relation.structureId)//（部门id）
                this.department = parseInt(add_relation.structureId)
                this.formLabelAlign.rankId = parseInt(add_relation.rankId)//职级
                this.rank = parseInt(add_relation.rankId);
                this.formLabelAlign.entryTime = add_relation.entryTime//入职时间（入职时间）
                this.formLabelAlign.probation = add_relation.probation,//(试用期时间)
                this.formLabelAlign.correctionTime = add_relation.correctionTime//（转正时间）
                this.textarea = add_relation.textarea//（岗位理解）
                this.summary = add_relation.summary//（试用期总结）
                this.suggestions = add_relation.suggestions//（意见建议）
                this.approvalId = parseInt(add_relation.approvalId)//（审批人ID）
            }
